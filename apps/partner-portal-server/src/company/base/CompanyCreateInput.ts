/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsString, IsOptional, ValidateNested } from "class-validator";
import { LicenseCreateNestedManyWithoutCompaniesInput } from "./LicenseCreateNestedManyWithoutCompaniesInput";
import { Type } from "class-transformer";
import { ModuleModelCreateNestedManyWithoutCompaniesInput } from "./ModuleModelCreateNestedManyWithoutCompaniesInput";
import { UserCreateNestedManyWithoutCompaniesInput } from "./UserCreateNestedManyWithoutCompaniesInput";

@InputType()
class CompanyCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  address?: string | null;

  @ApiProperty({
    required: false,
    type: () => LicenseCreateNestedManyWithoutCompaniesInput,
  })
  @ValidateNested()
  @Type(() => LicenseCreateNestedManyWithoutCompaniesInput)
  @IsOptional()
  @Field(() => LicenseCreateNestedManyWithoutCompaniesInput, {
    nullable: true,
  })
  licenses?: LicenseCreateNestedManyWithoutCompaniesInput;

  @ApiProperty({
    required: false,
    type: () => ModuleModelCreateNestedManyWithoutCompaniesInput,
  })
  @ValidateNested()
  @Type(() => ModuleModelCreateNestedManyWithoutCompaniesInput)
  @IsOptional()
  @Field(() => ModuleModelCreateNestedManyWithoutCompaniesInput, {
    nullable: true,
  })
  modules?: ModuleModelCreateNestedManyWithoutCompaniesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => UserCreateNestedManyWithoutCompaniesInput,
  })
  @ValidateNested()
  @Type(() => UserCreateNestedManyWithoutCompaniesInput)
  @IsOptional()
  @Field(() => UserCreateNestedManyWithoutCompaniesInput, {
    nullable: true,
  })
  users?: UserCreateNestedManyWithoutCompaniesInput;
}

export { CompanyCreateInput as CompanyCreateInput };
